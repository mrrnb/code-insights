...
DEFAULT_PI_DISPATCH_MODE = "inline"
...
def get_pi_dispatch_mode(repo_root: Path | None = None) -> str:
    """Return Pi dispatch mode, defaulting invalid values to inline."""
    config = _load_config(repo_root)
    pi = config.get("pi")
    if pi is None:
        return DEFAULT_PI_DISPATCH_MODE
    if not isinstance(pi, dict):
        print(f"[WARN] invalid pi config: {pi!r}; using inline", file=sys.stderr)
        return "inline"

    raw = pi.get("dispatch_mode", DEFAULT_PI_DISPATCH_MODE)
    mode = str(raw).strip().lower()
    if mode in ("inline", "sub-agent"):
        return mode
    print(
        f"[WARN] invalid pi.dispatch_mode value: {raw!r}; using inline",
        file=sys.stderr,
    )
    return "inline"
