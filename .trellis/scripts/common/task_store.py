...
def _has_subagent_platform(repo_root: Path) -> bool:
    ...
    legacy ``sub-agent`` alias). Pi counts only in explicit sub-agent mode.
    Inline modes load context through skills, not JSONL.
    """
    for config_dir in _SUBAGENT_CONFIG_DIRS:
        if (repo_root / config_dir).is_dir():
            return True
    if (repo_root / _PI_CONFIG_DIR).is_dir():
        if get_pi_dispatch_mode(repo_root) == "sub-agent":
            return True
