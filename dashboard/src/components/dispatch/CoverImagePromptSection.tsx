...
export function CoverImagePromptSection({ title, tags, tldr, format }: CoverImagePromptSectionProps) {
  ...
  function handleCopy() {
    ...
    void navigator.clipboard.writeText(mutation.data.prompt).then(() => {
      ...
      toast.success(t('dispatch.promptCopied'));
    });
  }
  ...
  return (
    <div className="shrink-0 border-t px-4 py-3 space-y-2">
      <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('dispatch.coverImagePrompt')}</p>
      ...
      {mutation.isError && (
        <div className="space-y-2">
          <Alert variant="destructive" className="py-2">
            ...
            <AlertDescription>
              {mutation.error instanceof Error ? mutation.error.message : t('dispatch.promptFailed')}
            </AlertDescription>
          </Alert>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="gap-1.5" onClick={handleCopy}>
              ...
              {copied ? t('dispatch.copied') : t('dispatch.copyPrompt')}
            </Button>
            <button
              type="button"
