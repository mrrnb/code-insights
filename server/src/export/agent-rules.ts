...
export function formatAgentRules(sessions: SessionRow[], insights: InsightRow[]): string {
  if (sessions.length === 0) {
    return '# Agent Rules Export / Agent Rules 导出\n# Agent Rules 导出\n\n*No sessions selected.* / *未选择任何会话。*\n';
  }
  ...
  const lines: string[] = [
    `# Agent Rules Export / Agent Rules 导出`,
    `# Agent Rules 导出`,
    `> Generated by Code Insights from ${formatCount(sessions.length, 'session')} / 由 Code Insights 基于 ${sessions.length} 个已分析会话生成`,
    `> Project: ${projectName} | Date range: ${dateRange} / 项目：${projectName} | 时间范围：${dateRange}`,
    '',
  ];

  if (insights.length === 0) {
    lines.push(
      '> **Note:** No insights found. Run analysis first. / **提示：** 当前没有可用洞察。请先分析会话，再生成规则。',
    );
    return lines.join('\n');
  }
  ...
  if (techniques && techniques.length > 0) {
    ...
    for (const insight of techniques) {
      ...
      if (applicability) {
        lines.push(`- Applicability: ${applicability} / 适用范围：${applicability}`);
      }
      lines.push('');
    }
  }
  ...
  if (pqInsights && pqInsights.length > 0) {
    ...
    for (const insight of pqInsights) {
      ...
      if (findings && Array.isArray(findings)) {
        for (const f of findings.filter(f => f.type === 'deficit')) {
          ...
          avoidLines.push(`- AVOID: ${f.description ?? 'Detected prompting issue'}${category}${fix}`);
        }
        continue;
      }
    ...
    if (avoidLines.length > 0) {
      lines.push('## Prompt Patterns to Avoid / 需要避免的 Prompt 模式');
